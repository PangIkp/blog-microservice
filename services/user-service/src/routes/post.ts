...
export default async function postRoutes(app: FastifyInstance) {
  ...
  app.put("/posts/:id", { preHandler: [verifyToken] }, async (req: any, reply) => {
    try {
      ...
      if (post.authorId !== userId) {
        return reply.status(403).send({ error: "You are not allowed to edit this post" });
      }
  ...
  app.delete("/posts/:id", { preHandler: [verifyToken] }, async (req: any, reply) => {
    try {
      const { id } = req.params as { id: string };
      const userId = req.user.userId;
